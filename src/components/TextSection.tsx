...
export default function TextSection({
  ...
  return (
    ...
    <section
      className={`w-full relative flex flex-col-reverse py-8 px-8 xl:px-16 gap-8 lg:gap-16 justify-between items-center ${
        textLocation !== "left" ? "md:flex-row-reverse" : "md:flex-row"
      }`}
      aria-label={title}
    >
      {/* Text Section */}
      <motion.div
        ...
        className={`text-center items-center z-15 flex flex-col gap-6 w-full lg:w-3/5 ${
          textColorMap[textColor] || "text-primaryBrown"
        } ${textLocation === "right" ? "md:text-right md:items-end" : "md:text-left md:items-start"}`}
      >
        {/* Title */}
        <motion.h2
      ...
      {imageUrl && (
        <motion.div
          ...
          className="relative z-10 flex items-center justify-center w-full md:w-2/5 h-full"
        >
          <motion.img
            ...
            title={`${title} section image`}
            alt={image.alternativeText || `${title} section image`}
            className={`rounded-xl object-cover h-82 max-h-lg md:max-w-85 md:h-auto ${
